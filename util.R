...
get_clusters_entry = function(clusters, assignments_table, indel_assignments=NULL, sv_assignments=NULL, min_clonal_ccf=0.9, max_clonal_ccf=1.1, do_filter=T) {
  ...
  # }  
  snv_assignment_probs = assignments_table[,grepl("cluster_")]
  if (!is.null(indel_assignments)) { indel_assignment_probs = indel_assignments[,grepl("cluster_")] }
  if (!is.null(sv_assignments)) { sv_assignment_probs = sv_assignments[,grepl("cluster_")] }
